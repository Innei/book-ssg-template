...
export const HeaderLogoArea: Component = ({ children }) => {
  return (
    <div className={clsxm("relative", styles["header--grid__logo"])}>
      <div className={clsxm("relative flex size-full items-center justify-center")}>{children}</div>
    </div>
  )
}
...
export const HeaderLeftButtonArea: Component = ({ children }) => {
  return (
    <div className={clsxm("relative flex size-full items-center justify-center xl:hidden")}>
      {children}
    </div>
  )
}
